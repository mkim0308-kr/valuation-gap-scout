...
- **FCF 이력은 10-K 연간 실적이 아니라 최근 5년 분기(10-Q/10-K) 데이터를
  ...
  TTM으로 합산하는 방식으로 바꿔 이 문제를 해결했습니다. 이 변경은
  NVDA만 고친 게 아니라 거의 모든 티커의 DCF 결과에 영향을 줍니다 —
  분기 TTM 기준은 연간 기준보다 최근 변동성/모멘텀을 훨씬 예민하게
  반영합니다. SEC EDGAR XBRL 태그는 기업마다 명명이 조금씩 달라, 이
  방식으로도 일부 종목은 5년 전체 분기 데이터가 수집되지 않을 수
  있습니다.
- **성장률 가정은 단일 상수가 아니라 2단계(2-stage) 구조**입니다 —
  1~2년차는 `near_term_growth_rate_used`(애널리스트 forward 추정치가
  있으면 우선 사용, 없으면 5년 분기 추세 성장률로 대체)로 투영하고,
  이후 `growth_rate_schedule`을 따라 5년차에는 터미널 성장률까지 선형
  수렴합니다. 과거 추세 성장률(`5yr_quarterly_metrics.fcf_cagr`)도
  2점 비교(`simple_endpoint_cagr`) 대신 **전체 TTM 시계열에 대한
  로그선형 회귀**로 계산해, 특정 한두 분기의 이상치에 덜 휘둘리도록
  했습니다. 이 조합 덕분에 종목별로 실제 애널리스트 전망 차이가 결과에
  훨씬 뚜렷하게 반영됩니다 — 예를 들어 실제 확인된 사례로 META는
  yfinance의 근시일 실적 forward 추정치가 **-13.4%(음수)**라 적정가가
  크게 낮아지고, NVDA는 forward 추정치가 +214.5%로 매우 높지만 성장률
  상한(30%)에 걸려 오히려 자체 과거 추세(58.6%)보다 낮게 클램프되어
  반영됩니다. "괴리율이 다 같이 좋아지거나 나빠지는" 게 아니라 종목별
  펀더멘털 차이가 더 잘 드러나는 방향으로 바뀌었다는 뜻이며, 그 결과
  개별 종목의 괴리율은 이 변경 전후로 크게(때로는 부호까지) 달라질 수
  있습니다.
