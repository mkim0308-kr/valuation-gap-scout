...
Agent 2, 3, 4는 검색 없이 실행하면 근거가 부족한 항목을 `insufficient_data`로
표시하도록 지시되어 있습니다 — 모르는 걸 지어내지 않기 위함입니다. Agent
4(구루 관점)는 실제 인물의 의견이 아니라 **공개적으로 알려진 투자
방법론(공식/기준)을 데이터에 기계적으로 대입한 결과**라는 점을 항상 고지문에
명시하도록 제약되어 있습니다. CFO 리포트(Agent 5)는 항상 투자 자문 아님
